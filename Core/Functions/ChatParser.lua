...
local function ChatParserFunc(self, event, message, sender, language, _, _, _, _, _, _, _, _, guid)
	...
	if not ChatListeners[event] then
		return
	end

	if E.Core:IsBlackListMSG(message) then
		return
	end
	if not E.Core:FindLFM(message) then
		return
	end
	...
	if raidInfo.find and roles.find then
		...
		local rlFaction =
			language == "всеобщий" and "Alliance" or language == "орочий" and "Horde" or language ==
				"арго скорпидов" and "Renegade"
